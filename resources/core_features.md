...
Какова средняя масса тела пингвинов весом более 3000,0 граммов?
...
- count(column) подсчитывает ненулевые записи в столбце
- count(отдельный столбец) подсчитывает отдельные ненулевые записи

#### Упражнения
- Сколько разных масс тела содержится в наборе данных по пингвинам?
...
- Другие менеджеры баз данных этого не делают.
Например, PostgreSQL жалуется, что столбец необходимо использовать в функции агрегации.


#### Упражнения

- Объясните, почему в результатах предыдущего запроса перед строками для самок и самцов пингвинов имеется пустая строка.

- Напишите запрос, который показывает каждую уникальную массу тела в наборе данных о пингвинах и количество пингвинов, которые весят столько же.
...
Используйте having в аггрегирующих функциях вместо where