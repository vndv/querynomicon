...
- Компанда `where` фильтрует строки, полученные в результате выбора.
...
Используйте выражение в качестве имени для переименования
Дайте результату расчета осмысленное имя.
Также можно переименовывать столбцы без изменения.

#### Упражнения

Напишите один запрос, который вычисляет и возвращает:

Столбец под названием «что_где», в котором виды и острова каждого пингвина разделены одним пробелом.
Столбец bill_ratio, в котором указано соотношение длины купюры к глубине купюры.
...
Не 0 или пустая строка, а «Я не знаю»
...
- Используйте значение null и не null для безопасной обработки null.
